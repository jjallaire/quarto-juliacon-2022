...
-   Quarto is a ground-up reimagining of R Markdown that is fundamentally multi-language and multi-engine.
...
- Markdown created by Jon Gruber and Aaron Swartx as an easier way to compose HTML
...
- Better approach would be if Julia plotting packages could automaticaly detect when they are running in Quarto and set their defaults accordingly
...
2. [Jupyter Cache](https://jupyter-cache.readthedocs.io/en/latest/) provides trainsient caching of cell outputs for a document (if any of the cells in the document change then all of the cells will be re-executed).
...
- Aligned with architectual investments we had already made for Jupyter (e.g. kernel daemoninzation, execution result caching)
